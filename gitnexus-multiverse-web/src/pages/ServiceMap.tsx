...
export default function ServiceMap() {
  ...
  const explore = useCallback((id: string, seeds: SeedType, focus?: string) => {
    ...
    get(`/api/mv/graph/${id}/explore?${params}`)
      .then((r) => {
        setNodes(r.nodes || []);
        setEdges(r.edges || []);
        setSeedIds(r.seeds || []);
  ...
  useEffect(() => {
    ...
    const r = (d: any) => {
      if (isService) {
        if (d.label?.includes('Transport')) return 6;
    ...
    const node = g
      ...
      .on('click', (_e: any, d: any) => {
        if (isService) {
          ...
          setSeedType('ENTRYPOINT');
          navigate(`/map?service=${d.id}`, { replace: true });
          explore(d.id, 'ENTRYPOINT');
        } else focusNode(d.id);
      })
      .call(
        d3
    ...
    <div className="flex h-full">
      <div className="flex flex-1 flex-col">
        ...
        <div className="relative flex-1">
          ...
          {mode === 'services' && (
            <div className="border-border absolute bottom-3 left-3 space-y-0.5 rounded-lg border bg-surface/90 p-2 text-[10px]">
              <div className="flex items-center gap-1.5">
                <span className="h-3 w-3 rounded-full" style={{ background: '#6c5ce7' }} />
                Service
              </div>
              <div className="flex items-center gap-1.5">
                <span className="h-3 w-3 rounded-full" style={{ background: '#a29bfe' }} />
                Library
              </div>
              <div className="flex items-center gap-1.5">
                <span className="h-2 w-2 rotate-45" style={{ background: '#74b9ff' }} />
                API
              </div>
              <div className="flex items-center gap-1.5">
                <span className="h-2 w-2 rotate-45" style={{ background: '#ff7675' }} />
                Kafka
              </div>
              <div className="flex items-center gap-1.5">
                <span className="h-2 w-2 rotate-45" style={{ background: '#a29bfe' }} />
                Rabbit
              </div>
              <div className="flex items-center gap-1.5">
                <span className="h-2 w-2 rotate-45" style={{ background: '#e17055' }} />
                Redis
              </div>
              <div className="border-border/50 mt-1 flex items-center gap-1.5 border-t pt-1">
                <span className="w-4 border-t border-[#74b9ff]" />
                HTTP
              </div>
              <div className="flex items-center gap-1.5">
                <span className="w-4 border-t border-dashed border-[#ff7675]" />
                Kafka
              </div>
              <div className="flex items-center gap-1.5">
                <span className="w-4 border-t border-dashed border-[#a29bfe]" />
                Rabbit
              </div>
              <div className="flex items-center gap-1.5">
                <span className="w-4 border-t border-dashed border-[#e17055]" />
                Redis
              </div>
              <div className="flex items-center gap-1.5">
                <span className="w-4 border-t border-dashed border-[#f59e0b]" />
                ActiveMQ
              </div>
              <div className="flex items-center gap-1.5">
                <span className="w-4 border-t border-[#d4a017]" />
                SOAP
              </div>
              <div className="flex items-center gap-1.5">
                <span className="w-4 border-t border-dashed border-[#a29bfe]" />
