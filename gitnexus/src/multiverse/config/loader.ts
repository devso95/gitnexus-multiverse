 ...
 *
 * Loads multiverse-config.yml with ${ENV_VAR} substitution.
...
const DEFAULTS: MultiverseConfig = {
  ...
  auth: {
    users: [], // No default credentials — require env var or config
  },
  workspace: {
...
/** Simple YAML-subset parser — handles flat/nested keys, arrays, strings, numbers, booleans */
...
export const loadConfig = async (configPath?: string): Promise<MultiverseConfig> => {
  ...
  // Ensure auth.users is valid
  if (!config.auth?.users?.length) {
    config.auth = { ...config.auth, users: [] };
  }

  // Override auth from env vars (MV_ADMIN_USER / MV_ADMIN_PASS) — REQUIRED for first run
  ...
  if (envUser && envPass) {
    try {
      const bcrypt = (await import('bcryptjs')) as unknown as {
        default?: { hashSync(s: string, n: number): string };
        hashSync?(s: string, n: number): string;
      };
      const bc = bcrypt.default || bcrypt;
      if (typeof bc.hashSync === 'function') {
        const hash = bc.hashSync(envPass, 10);
        config.auth.users = [{ username: envUser, password: hash, role: 'admin' as const }];
      }
    } catch {
  ...
  if (!config.workspace?.dir) mvLog.warn(LOG, 'workspace.dir not configured — using default');
